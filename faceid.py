...
default_model_name = "faceid-facemobilenet"
default_image_size = 112
default_model_alpha = 1.0
...
class FaceID():
    def __init__(self, frame_w, frame_h, inferencer=None, model_name=None, image_size=None, model_alpha=None, model_precision=None, inference_device=None):
        ...
        try:
            ...
            if not(self.model_name is None):
                ...
                if self.model_name.split('-')[0] != faceid_prefix:
    ...
    def feautures(self, frame, faces):
        resized_images = []

        for face in faces:
            resized_image = np.ones((self.image_size, self.image_size, 3), dtype=np.uint8) * fill_color
            ...
            resized_image[shift_y:(shift_y + crop_h), shift_x:(shift_x + crop_w)] = cv2.resize(frame[crop_y1:crop_y2, crop_x1:crop_x2], (crop_w, crop_h), interpolation=cv2.INTER_NEAREST)
            resized_images.append(resized_image)

        resized_images = np.array(resized_images, dtype=np.uint8)
        if len(resized_images) > 0:
            feautures = self.model.predict(resized_images, batch_size=min(batch_size, len(resized_images)), verbose=0)
            feautures /= np.sqrt(np.maximum(np.sum(feautures ** 2, axis=-1).reshape(feautures.shape[0], 1), 1e-12))
        else:
            feautures = []

        return np.array(feautures)
